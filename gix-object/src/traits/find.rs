...
/// Find an object in the object store, asynchronously
///
/// ## Notes
///
...
mod ext {
    ...
    macro_rules! make_async_obj_lookup {
        ($method:ident, $object_variant:path, $object_kind:path, $object_type:ty) => {
            /// Like [`find(…)`][Self::find()], but flattens the `Result<Option<_>>` into a single `Result` making a non-existing object an error
            /// while returning the desired object type.
            async fn $method<'a>(
    ...
    macro_rules! make_async_iter_lookup {
        ($method:ident, $object_kind:path, $object_type:ty, $into_iter:tt) => {
            /// Like [`find(…)`][Self::find()], but flattens the `Result<Option<_>>` into a single `Result` making a non-existing object an error
            /// while returning the desired iterator type.
            async fn $method<'a>(
    ...
    /// An extension trait with convenience functions.
    #[async_trait::async_trait]
    pub trait AsyncFindExt: super::AsyncFind {
        /// Like [`try_find(…)`](super::Find::try_find()), but flattens the `Result<Option<_>>` into a single `Result` making a non-existing object an error.
        async fn find<'a>(
            &self,
            id: &gix_hash::oid,
        ...
        make_async_obj_lookup!(find_commit, ObjectRef::Commit, Kind::Commit, CommitRef<'a>);
        make_async_obj_lookup!(find_tag, ObjectRef::Tag, Kind::Tag, TagRef<'a>);
        make_async_iter_lookup!(find_commit_iter, Kind::Commit, CommitRefIter<'a>, try_into_commit_iter);
        make_async_iter_lookup!(find_tree_iter, Kind::Tree, TreeRefIter<'a>, try_into_tree_iter);
        make_async_iter_lookup!(find_tag_iter, Kind::Tag, TagRefIter<'a>, try_into_tag_iter);
    }

    impl<T: super::AsyncFind + ?Sized> AsyncFindExt for T {}
    impl<T: super::Find + ?Sized> FindExt for T {}
}
pub use ext::{AsyncFindExt, FindExt, HeaderExt};
