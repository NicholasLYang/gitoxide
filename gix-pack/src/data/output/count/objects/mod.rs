...
mod expand_async {
    ...
    use super::expand::push_obj_count_unique;
    use super::{
        tree,
    ...
    async fn id_to_count(
        ...
        output::Count {
            ...
            entry_pack_location: if allow_pack_lookups {
                PackLocation::LookedUp(db.location_by_oid(id, buf).await)
            } else {
    ...
    pub async fn this(
        ...
        for id in oids {
            ...
            let (obj, location) = db.find(&id, buf1).await?;
                        ...
                        match obj.kind {
                            ...
                            Commit => {
                                let current_tree_iter = {
                                    ...
                                    let (obj, location) = db.find(&tree_id, buf1).await?;
                                ...
                                let objects_ref = if parent_commit_ids.is_empty() {
                                    ...
                                    gix_traverse::tree::breadthfirst_async(
                                        current_tree_iter,
                                    ...
                                    for commit_id in &parent_commit_ids {
                                        let parent_tree_id = {
                                            let (parent_commit_obj, location) = db.find(commit_id, buf2).await?;
                                        ...
                                        let parent_tree = {
                                            let (parent_tree_obj, location) = db.find(&parent_tree_id, buf2).await?;
                                            push_obj_count_unique(
                                                &mut out,
                                                seen_objs,
                                                &parent_tree_id,
                                ...
                                for id in objects_ref.iter() {
                                    out.push(id_to_count(db, buf2, id, objects, stats, allow_pack_lookups).await);
                                }
                                break;
                            }
                        }
                        ...
                        match obj.0.kind {
                            Tree => {
                                ...
                                for id in &traverse_delegate.non_trees {
                                    out.push(id_to_count(db, buf1, id, objects, stats, allow_pack_lookups).await);
                                }
                                break;
                            }
                            Commit => {
    ...
    struct CountingObjects<'a> {
        decoded_objects: Mutex<RefCell<usize>>,
        objects: &'a dyn crate::AsyncFind,
    }

    impl<'a> CountingObjects<'a> {
        fn new(objects: &'a dyn crate::AsyncFind) -> Self {
            Self {
        ...
        fn into_count(self) -> usize {
            self.decoded_objects.into_inner().into_inner()
        }
    }

    #[async_trait::async_trait]
    impl gix_object::AsyncFind for CountingObjects<'_> {
        async fn try_find<'a>(
            &self,
            id: &oid,
            buffer: &'a mut Vec<u8>,
        ) -> Result<Option<Data<'a>>, gix_object::find::Error> {
            let res = Ok(self.objects.try_find(id, buffer).await?.map(|t| t.0));
            *self.decoded_objects.lock().borrow_mut() += 1;
            res
        }
    }

    struct ExpandedCountingObjects<'a> {
        decoded_objects: AtomicUsize,
        expanded_objects: AtomicUsize,
        out: Mutex<RefCell<Vec<output::Count>>>,
    ...
    impl<'a> ExpandedCountingObjects<'a> {
        ...
        fn dissolve(self, stats: &mut Outcome) -> Vec<output::Count> {
            stats.decoded_objects += self.decoded_objects.load(Ordering::Relaxed);
            stats.expanded_objects += self.expanded_objects.load(Ordering::Relaxed);
            self.out.into_inner().into_inner()
        }
    }

    #[async_trait::async_trait]
    impl gix_object::AsyncFind for ExpandedCountingObjects<'_> {
        async fn try_find<'a>(
            &self,
            id: &oid,
            buffer: &'a mut Vec<u8>,
        ) -> Result<Option<Data<'a>>, gix_object::find::Error> {
            let maybe_obj = self.objects.try_find(id, buffer).await?;
            self.decoded_objects.fetch_add(1, Ordering::Relaxed);
            match maybe_obj {
                None => Ok(None),
                Some((obj, location)) => {
                    self.objects_count.fetch_add(1, Ordering::Relaxed);
                    self.expanded_objects.fetch_add(1, Ordering::Relaxed);
                    self.out
                        .lock()
                        .borrow_mut()
                        .push(output::Count::from_data(id, location));
                    Ok(Some(obj))
                }
            }
        }
...
mod expand {
    ...
    #[inline]
    pub(crate) fn push_obj_count_unique(
